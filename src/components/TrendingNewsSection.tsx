...
// Custom hook to get responsive items per slide
...
export default function TrendingNewsSection({ articles }: TrendingNewsSectionProps) {
    ...
    // Reset index when items per slide changes (screen resize)
    useEffect(() => {
        setCurrentIndex(0);
    }, [itemsPerSlide]);
                    ...
                    <div className="hidden sm:flex gap-2">
                        {Array.from({ length: totalSlides }).map((_, i) => (
                            <button
                                key={i}
                                onClick={() => goToSlide(i)}
                                className={`h-2 rounded-full transition-all duration-500 ease-out ${i === currentIndex ? 'bg-secondary w-8' : 'bg-neutral-300 hover:bg-neutral-400 w-2'
                                    }`}
                            />
                        ))}
                    </div>
                                    ...
                                    >
                                        ...
                                        <div className="p-5">
                                            ...
                                            <div className="flex items-center gap-2 text-xs text-neutral-500 mb-3">
                                                <img src="/1.png" alt="Fista" className="h-4 w-auto -ml-2" />
            ...
            <div className="flex sm:hidden justify-center gap-2 mt-6">
                {Array.from({ length: totalSlides }).map((_, i) => (
                    <button
                        key={i}
                        onClick={() => goToSlide(i)}
                        className={`h-2 rounded-full transition-all duration-500 ease-out ${i === currentIndex ? 'bg-secondary w-8' : 'bg-neutral-300 w-2'
                            }`}
                    />
                ))}
            </div>
